...
class Game():
    def __init__(self):
        ...
        self.notes_2 = []
        self.notes_3 = []
        self.notes_0.append(Note(0, 2))
        self.notes_1.append(Note(1, 2))
        self.notes_2.append(Note(2, 2))
    ...
    def note_decesion(self, note_ypos, line): # 노트 판정
        ...
        if self.gap >= 50: # fail
            ...
            self.hp -= 5
            print(self.score)
        elif self.gap < 50 and self.gap >= 30: # normal
            self.score += 1
            print(self.score)
        elif self.gap < 30 and self.gap >= 20: # great
            self.score += 5
            print(self.score)
        elif self.gap < 10 and self.gap >= 0 : # perfect
            self.score += 10    
            print(self.score)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_d: # d 키를 누름
                    self.pressed_d = True
                    for note in self.notes_0:
                        if abs(self.line - note.rect.y) <= 100 and note.lane == 0:
                ...
                if event.key == pygame.K_f: # f 키를 누름
                    self.pressed_f = True
                    for note in self.notes_1:
                        if abs(self.line - note.rect.y) <= 100 and note.lane == 1:
                            self.note_decesion(note.rect.y, self.line)
                            del self.notes_1[0]
                ...
                if event.key == pygame.K_j: # j 키를 누름
                    self.pressed_j = True
                    for note in self.notes_2:
                        if abs(self.line - note.rect.y) <= 100 and note.lane == 2:
                            self.note_decesion(note.rect.y, self.line)
                            del self.notes_2[0]
                ...
                if event.key == pygame.K_k: # k 키를 누름
                    self.pressed_k = True
                    for note in self.notes_3:
                        if abs(self.line - note.rect.y) <= 100 and note.lane == 3:
                            self.note_decesion(note.rect.y, self.line)
                            del self.notes_3[0]
